...
export function UsdToVef() {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        const response = await fetch('https://budget-backend-railway-production.up.railway.app/add_exchange_usd', {
          method: 'POST',
          credentials: 'include',
          headers: {
      ...
      <>
        ...
        <Modal show={show} onHide={handleClose}>
          <Modal.Header closeButton>
            <Modal.Title>Create a new transfer (USD TO VEF)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
  ...
  export function VefToUsd() {
    ...
    const dispatch = useDispatch();
    const { accounts } = useSelector(state => state.accounts)
    const acc = Object.values(accounts || [])
    ...
    const handleSubmit = async (e) => {
      ...
      const token = window.localStorage.getItem('authToken')
      try {
        const response = await fetch('https://budget-backend-railway-production.up.railway.app/add_exchange_vef', {
          method: 'POST',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
    ...
    return (
      <>
        ...
        <Modal show={show} onHide={handleClose}>
          <Modal.Header closeButton>
            <Modal.Title>Create a new transfer (VEF TO USD)</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <Form onSubmit={handleSubmit}>
              <Form.Group className="mb-3">
